...
Search first runs a deterministic, token-based Sanity query over the compact course and lesson index. It returns matching lesson topics plus chapter-first video moments quickly and without sending transcript arrays to a model. The Context MCP and OpenAI agent are a bounded semantic-expansion fallback only when the direct search has no grounded matches.
...
Then verify that the MCP endpoint exposes `initial_context`, `groq_query`, and schema-reading tools. Timestamped cards require the offline ingestion pipeline to have created a `video` document whose provider video ID matches the lesson URL.
