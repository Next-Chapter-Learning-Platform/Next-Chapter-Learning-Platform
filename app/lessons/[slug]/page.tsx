...
function moduleDuration(courseModule: CourseModule) {
  return courseModule.lessons.reduce((total, lesson) => total + lesson.duration, 0);
}
          ...
          return (
            <details className={styles.module} open={isCurrentModule} key={courseModule._key}>
              <summary>
                ...
                <span className={styles.moduleTitle}><strong>{courseModule.title}</strong><small>{formatDuration(moduleDuration(courseModule))}</small></span>
                  ...
                  return (
                    <li className={isCurrentLesson ? styles.currentLesson : ""} key={moduleLesson._key}>
                      ...
                      <Link href={`/lessons/${moduleLesson.slug}`} aria-current={isCurrentLesson ? "page" : undefined}>
                        ...
                        <small>{isCurrentLesson ? "Now playing" : formatDuration(moduleLesson.duration)}</small>
                      </Link>
                    </li>
                  );
                })}
...
export default async function LessonPage({ params, searchParams }: PageProps<"/lessons/[slug]">) {
  ...
  const videoUrl = videoId
    ? `https://www.youtube-nocookie.com/embed/${videoId}?rel=0${startSeconds > 0 ? `&start=${startSeconds}` : ""}`
    : null;
      ...
      <div className={styles.pageFrame}>
            ...
            <header className={styles.lessonHeader}>
              <div>
                ...
                <div className={styles.meta} aria-label="Lesson details">
                  <MetaItem icon="clock">{formatDuration(lesson.duration)}</MetaItem>
            ...
            <section className={styles.videoSection} aria-label="Lesson video">
              {videoUrl ? (
                ...
                <div className={styles.videoUnavailable}>
                  {posterUrl && <Image src={posterUrl} alt={lesson.thumbnail.alt} fill sizes="(max-width: 900px) calc(100vw - 40px), 900px" placeholder={posterBlurDataUrl ? "blur" : "empty"} blurDataURL={posterBlurDataUrl} />}
        ...
        <footer className={styles.lessonNavigation}>
          <div className={styles.previousAction}>
            {course.previousLesson ? <Link href={`/lessons/${course.previousLesson.slug}`}><Icon name="back" size={18} />Previous Lesson</Link> : <span className={styles.disabledAction}>First lesson</span>}
          </div>
          <div className={styles.previousTitle}>
            {course.previousLesson && <><span>{course.previousLesson.title}</span><small>{formatDuration(course.previousLesson.duration)}</small></>}
          </div>
          <div className={styles.nextTitle}>
            {course.nextLesson && <><span>{course.nextLesson.title}</span><small>{formatDuration(course.nextLesson.duration)}</small></>}
          </div>
          <div className={styles.nextAction}>
            {course.nextLesson ? <Link href={`/lessons/${course.nextLesson.slug}`}>Next Lesson<Icon name="arrow" size={19} /></Link> : <span className={styles.disabledAction}>Course complete</span>}
          </div>
        </footer>
      </div>
    </div>
